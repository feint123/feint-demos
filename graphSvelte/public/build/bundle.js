...
var app = (function () {
    ...
    ***/
     ...
     * 连线节点
     */
    class Connection extends VNode {
      constructor(id) {
        super(id);
        this.renderOnce();
      }
    ...
    class MindNode extends Rectangle {
    ...
    class MindApp extends GraphApp {
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			t2 = space();
    			div1 = element("div");
    			add_location(button, file, 125, 4, 4656);
    			attr_dev(div0, "class", "app svelte-c6expf");
    			add_location(div0, file, 126, 4, 4702);
    			attr_dev(div1, "class", "app-2 svelte-c6expf");
    			add_location(div1, file, 127, 4, 4730);
    			attr_dev(main, "class", "svelte-c6expf");
    			add_location(main, file, 124, 0, 4645);
    		},
    		...
    		m: function mount(target, anchor) {
    			...
    			if (!mounted) {
    				dispose = listen_dev(button, "click", /*saveData*/ ctx[0], false, false, false, false);
    				mounted = true;
    			}
    		},
    		p: noop$1,
    		...
    		mindApp.enableKeyDown(ev => {
    			if (ev.key === "Tab") {
    				...
    				if (mindApp.selectNodes && mindApp.selectNodes.length === 1) {
    					...
    					const newNode = new MindNode(new Date().getTime(), currentNode.level + 1);
    					newNode.content = "";
    				...
    				if (mindApp.startInputing) {
    					mindApp.clearInput();
    					mindApp.render();
    				} else if (mindApp.selectNodes && mindApp.selectNodes.length === 1) {
    					const currentNode = mindApp.selectNodes[0];

    					if (currentNode.level >= 1) {
    						const parentLevelNodes = mindApp.vnodes.filter(node => node.level === currentNode.level - 1).filter(node => node.connectToNodes.includes(currentNode));

    						if (parentLevelNodes && parentLevelNodes.length == 1) {
    							const newNode = new MindNode(new Date().getTime(), currentNode.level);
    							mindApp.addNode(newNode);
    				...
    				if (!mindApp.startInputing && mindApp.selectNodes && mindApp.selectNodes.length === 1) {
    					if (ev.key === "Backspace") {
     ...
     */
    			...
    			if (node.parentNodes.length > 0) {
    				node.parentNodes.forEach(pNode => {
    					if (mindApp.vnodes.includes(pNode)) {
    						pNode.connectToNodes.splice(pNode.connectToNodes.indexOf(node), 1);
    					}
    				});
    			}
    	...
    	$$self.$inject_state = $$props => {
    		if ('mindApp' in $$props) mindApp = $$props.mindApp;
    	};
